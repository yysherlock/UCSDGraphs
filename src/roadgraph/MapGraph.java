...
public class MapGraph {
	...
	public List<GeographicPoint> dijkstraArg(GeographicPoint start, GeographicPoint goal, boolean argumentOrNot) {
		// Dummy variable for calling the search algorithms
		// You do not need to change this method.
		Consumer<GeographicPoint> temp = (x) -> {
		};
		return dijkstraArg(start, goal, temp, argumentOrNot);
	}

	public List<GeographicPoint> dijkstraArg(GeographicPoint start, GeographicPoint goal,
			Consumer<GeographicPoint> nodeSearched, boolean argumentOrNot) {
		...
		while (!pqueue.isEmpty()) {
			...
			if (cur.equals(endNode)) {
				// cur.setActualDistance(cur.getDistance());
				break;
			}
			if (!visited.contains(cur)) {
				...
				for (MapEdge edge : cur.getEdges()) {
					...
					if (!visited.contains(neighbor)) {
						if (cur.getDistance() + edge.getLength(argumentOrNot) < neighbor.getDistance()) {
							neighbor.setDistance(cur.getDistance() + edge.getLength(argumentOrNot));
							pqueue.add(neighbor);
		...
		while (!pqueue.isEmpty()) {
			...
			if (!visited.contains(cur)) {
				...
				for (MapEdge edge : cur.getEdges()) {
					MapNode neighbor = edge.getEndNode();
					if (!visited.contains(neighbor)) {
						if (cur.getDistance() + edge.getLength() < neighbor.getDistance()) {
							neighbor.setDistance(cur.getDistance() + edge.getLength());
							pqueue.add(neighbor);
							parentMap.put(neighbor, cur);
	...
	public List<GeographicPoint> aStarSearchArg(GeographicPoint start, GeographicPoint goal,
			boolean ArgumentOrNot) {
		// Dummy variable for calling the search algorithms
		Consumer<GeographicPoint> temp = (x) -> {
		};
		return aStarSearchArg(start, goal, temp, ArgumentOrNot);
	}
		...
		while (!pqueue.isEmpty()) {
			...
			if (!visited.contains(cur)) {
				...
				for (MapEdge edge : cur.getEdges()) {
					MapNode neighbor = edge.getEndNode();
					if (!visited.contains(neighbor)) {
						if (cur.getActualDistance() + edge.getLength(argumentOrNot) < neighbor.getActualDistance()) {
							neighbor.setActualDistance(cur.getActualDistance() + edge.getLength(argumentOrNot));
							double hestimate = neighbor.getLocation().distance(endNode.getLocation());
							neighbor.setDistance(neighbor.getActualDistance() + hestimate);
							pqueue.add(neighbor);
							parentMap.put(neighbor, cur);
		...
		while (!pqueue.isEmpty()) {
			...
			if (!visited.contains(cur)) {
				...
				for (MapEdge edge : cur.getEdges()) {
					MapNode neighbor = edge.getEndNode();
					if (!visited.contains(neighbor)) {
						if (cur.getActualDistance() + edge.getLength() < neighbor.getActualDistance()) {
							neighbor.setActualDistance(cur.getActualDistance() + edge.getLength());
							double hestimate = neighbor.getLocation().distance(endNode.getLocation());
							neighbor.setDistance(neighbor.getActualDistance() + hestimate);
							pqueue.add(neighbor);
							parentMap.put(neighbor, cur);
	...
	public static void main(String[] args) {
		...
		List<GeographicPoint> route0 = theMap.dijkstraArg(start, end,false);
		List<GeographicPoint> route1 = theMap.dijkstraArg(start, end,true);
		List<GeographicPoint> route2 = theMap.aStarSearch(start, end);
		List<GeographicPoint> route3 = theMap.aStarSearchArg(start, end, false);
		List<GeographicPoint> route4 = theMap.aStarSearchArg(start, end, true);
